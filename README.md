...
- **配置备份 / 恢复**：支持导出通道、HTTPS 代理、MAC 地址簿与 frpc 配置；恢复时采用合并导入，不删除已有配置
...
- 导出：下载包含通道列表、HTTPS 代理规则、MAC 地址簿和当前 frpc 配置的 JSON 备份文件
- 恢复：上传备份 JSON 后采用合并导入，已有配置会保留，备份中没有的配置不会删除
- 合并规则：通道按“名称 + 类型”匹配，HTTPS 代理按监听端口匹配，MAC 地址簿按 MAC 地址匹配；已存在则更新，不存在则追加
